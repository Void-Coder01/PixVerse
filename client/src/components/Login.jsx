...
const Login = () => {
    ...
    const {setShowLogin, backendurl, setToken, setIsuserLoggedIn} = useContext(AppContext);
    ...
    const onSubmitHandler = async (e) => {
        {/* it will prevent from re-loading when we submit the form */}
        e.preventDefault();

        {/* logic to handle the login in existed account else create a new one */}
        try {
            ...
            if(state === 'Login'){
                ...
                if(data.success){
                    ...
                    {/* to hidden the loginForm */}
                    setShowLogin(false)
                }else{
                    toast.error(data.message)
                }
            }else{
                {/* if user is not registered create new acc */}
    ...
    return (
        ...
        viewport={{once:true}}
        className="fixed top-0 left-0 right-0 bottom-0 z-10 backdrop-blur-sm bg-black/30 flex 
